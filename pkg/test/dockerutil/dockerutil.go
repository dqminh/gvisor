...
func MakeDocker(logger testutil.Logger) *Docker {
	return &Docker{
		logger:  logger,
		Name:    testutil.UniqueID(logger.Name()),
		Runtime: *runtime,
	}
}
...
func (d *Docker) CopyFiles(target string, sources ...string) {
	...
	for _, name := range sources {
		...
		if err := testutil.Copy(src, dst); err != nil {
			d.copyErr = fmt.Errorf("testutil.Copy(%q, %q) failed: %v", src, d.logger, err)
			return
		}
...
func (d *Docker) Logs() (string, error) {
	// Don't captuer the output; since it will swamp the logs.
